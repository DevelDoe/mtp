    ...
    if (ev == MG_EV_HTTP_MSG) {
        ...
        if (mg_http_match_uri(hm, "/")) {
            ...
            if (msg && json_object_object_get_ex(msg, "client_id", &client_id_obj) &&
                ...
                // Extract the structure of the JSON and count elements
if (json_object_get_type(data_obj) == json_type_array) {
    int count = json_object_array_length(data_obj);
    printf("[SERVER] Received symbol update from %s | %d elements\n", client_id, count);

    // Print structure of the JSON data
    printf("[SERVER] JSON Structure: %s\n", json_object_to_json_string_ext(data_obj, JSON_C_TO_STRING_PRETTY));
} else {
    printf("[SERVER] Received symbol update from %s | Not an array\n", client_id);
}
